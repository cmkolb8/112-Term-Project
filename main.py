...
#this file has all of the basic UI for the modes
#start screen mode 
class StartScreenMode(Mode): 
    def appStarted(mode):
        ...
        mode.statsColor = 'red3'
        mode.statsOutline = 'red3'
        mixer.init()
    ...
    def mousePressed(mode, event):  
        ...
        if(mode.width/2 - 75 <= event.x <= mode.width/2 + 75 
            ...
            mode.gameColor = 'red3'
            mode.gameOutline = 'red3'
        if(mode.width - 120 <= event.x <= mode.width - 20
            and mode.height - 50 <= event.y <= mode.height - 20):
    ...
    def redrawAll(mode, canvas):
        ...
        canvas.create_rectangle(mode.width - 120, mode.height - 50, mode.width - 20, mode.height - 20, fill = mode.statsColor, outline = mode.statsOutline)
        canvas.create_text(mode.width - 70, mode.height - 35, text = 'Stats', fill = 'white', font = 'Times 26 bold')
...
class StatsMode(Mode):
    ...
    def mouseMoved(mode, event):
        if(20 <= event.x <= 120 and mode.height - 50 <= event.y <= mode.height - 20):
            mode.color = 'red'
            mode.outline = 'white'
        else: 
            mode.color = 'red3'
    ...
    def redrawAll(mode, canvas):
        canvas.create_rectangle(0, 0, mode.width, mode.height, fill = 'black')
        stars.stars(mode, canvas)
        text = scoreBoard.readFile("highScore.txt")
        canvas.create_text(mode.width/2, mode.height/2, text = text, fill = 'white', font = 'Times 26 bold')
        canvas.create_rectangle(20, mode.height - 50, 120, mode.height - 20, fill = mode.color, outline = mode.outline)
        canvas.create_text(70, mode.height - 35, text = 'return', fill = 'white', font = 'Times 26 bold')
...
class InstructionsMode(Mode):
    ...
    def redrawAll(mode, canvas):
        ...
        canvas.create_text(mode.width / 2, mode.height / 2, text ='Walk around your side setting up 10 targets (the green blocks).'
                ...
                ' The first team to destroy all the other teams green blocks wins!'
                ' If you click t, you will see a top view of the game. You may then click r to return to the game',
                 fill = 'firebrick3', font = 'Times 20 bold', width = mode.width - 50)
